...
def test_classification_without_extracted_data():
    ...
    classified = ClassifiedInput(
        category=Category.UNKNOWN, confidence=0.2, raw_input="Something unclear"
    )
...
def test_confidence_minimum_valid():
    ...
    classified = ClassifiedInput(category=Category.UNKNOWN, confidence=0.0, raw_input="No idea")
...
def test_confidence_maximum_valid():
    ...
    classified = ClassifiedInput(
        category=Category.EXPENSE, confidence=1.0, raw_input="Clear expense"
    )
...
def test_confidence_below_minimum():
    ...
    with pytest.raises(ValidationError) as exc_info:
        ClassifiedInput(category=Category.EXPENSE, confidence=-0.1, raw_input="Test")
...
def test_confidence_above_maximum():
    ...
    with pytest.raises(ValidationError) as exc_info:
        ClassifiedInput(category=Category.EXPENSE, confidence=1.5, raw_input="Test")
...
def test_missing_category():
    ...
    with pytest.raises(ValidationError) as exc_info:
        ClassifiedInput(confidence=0.9, raw_input="Test input")
...
def test_missing_confidence():
    ...
    with pytest.raises(ValidationError) as exc_info:
        ClassifiedInput(category=Category.EXPENSE, raw_input="Test input")
...
def test_missing_raw_input():
    ...
    with pytest.raises(ValidationError) as exc_info:
        ClassifiedInput(category=Category.EXPENSE, confidence=0.9)
...
def test_invalid_category():
    ...
    with pytest.raises(ValidationError) as exc_info:
        ClassifiedInput(category="invalid_category", confidence=0.9, raw_input="Test")
